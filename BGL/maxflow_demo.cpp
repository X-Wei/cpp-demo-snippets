...
typedef adjacency_list<vecS, vecS, directedS, 
                        no_property,
                        property<edge_capacity_t, long, // edge property
...
int main(){
    ...
    ea.addEdge(2, 3, 1);
    // Add source and sink
    Vertex _source = add_vertex(G); // not to confuse with the source function!!
    Vertex _sink = add_vertex(G);
    ea.addEdge(_source, 0, 2);
    ea.addEdge(_source, 2, 1);
    ea.addEdge(1, _sink, 2);
    ea.addEdge(3, _sink, 1);
    // Calculate flow
    long flow1 = edmonds_karp_max_flow(G, _source, _sink);
    cout << "Maximum Flow with the Edmonds-Karp Algorihtm: " << flow1 << endl;
    long flow2 = push_relabel_max_flow(G, _source, _sink);
    cout << "Maximum Flow with the Push-Relabel Algorihtm: " << flow2 << endl;
    EdgeIt e, iend;
    for(tie(e,iend)=edges(G); e!=iend; ++e){
